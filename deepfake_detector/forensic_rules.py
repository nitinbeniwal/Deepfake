...
def noise_score(path):
    ...
    noise = f - cv2.GaussianBlur(f, (5, 5), 0)
    std = float(np.std(noise))
    if std < 1.5:  s = min(100, (1.5 - std) * 40 + 20)
    elif std > 25: s = min(100, (std - 25) * 2 + 15)
    else:          s = 0
    h, w = img.shape
    qs = [float(np.std(noise[r*h//2:(r+1)*h//2, c*w//2:(c+1)*w//2]))
          for r in range(2) for c in range(2)]
    if qs and np.std(qs) > std * 0.8:
        s = min(100, s + 15)
...
def boundary_score(path):
    ...
    h, w = img.shape
    bh, bw = max(4, int(h * 0.15)), max(4, int(w * 0.15))
...
def ela_score(path):
    """Error Level Analysis — detects JPEG re-compression from deepfake pipeline."""
    try:
        ...
        rec    = np.array(recomp, dtype=np.float32)
        ela    = np.abs(orig - rec)
        ela_mean = float(np.mean(ela))
        ela_std  = float(np.std(ela))
        # Deepfake images have inconsistent re-compression regions
        s = 0.0
        if ela_std  > 18: s += min(60, (ela_std  - 18) * 2.5 + 30)
        if ela_mean > 10: s += min(40, (ela_mean - 10) * 3.0 + 15)
        return round(float(min(100, s)), 1)
    except Exception:
        return 0.0


def gradient_score(path):
    """
    Gradient magnitude distribution.
    Real faces: natural gradient variation.
    GAN faces: unnaturally smooth or over-sharpened gradients.
    """
    img = _g(path)
    if img is None: return 0
    ...
    std_g  = float(np.std(mag))
    cv_g   = std_g / (mean_g + 1e-8)
    # Very low CV → unnaturally smooth (GAN over-smoothing)
    # Very high CV → over-sharpened edges (GAN artifacts)
    s = 0.0
    if cv_g < 0.8:  s = min(60, (0.8 - cv_g) * 75)
    elif cv_g > 3.0: s = min(40, (cv_g - 3.0) * 15 + 15)
    # Also check gradient symmetry (real faces are roughly symmetric)
    h, w = img.shape
    left  = float(np.mean(mag[:, :w//2]))
    right = float(np.mean(mag[:, w//2:]))
    asym  = abs(left - right) / (max(left, right) + 1e-8)
    if asym > 0.35: s = min(100, s + asym * 30)
    return round(float(s), 1)
...
def color_score(paths):
    ...
    for p in paths[:20]:
        ...
        cb.append(float(np.mean(y[:, :, 1])))
        cr.append(float(np.mean(y[:, :, 2])))
...
def temporal_score(paths):
    imgs = [cv2.resize(_g(p), (64, 64)).astype(np.float32)
            for p in paths[:15] if _g(p) is not None]
    if len(imgs) < 3: return 0
    diffs  = [float(np.mean(np.abs(imgs[i] - imgs[i-1]))) for i in range(1, len(imgs))]
    mean_d = float(np.mean(diffs))
    cv     = float(np.std(diffs)) / (mean_d + 1e-8)
    s = 0
    if cv > 1.5:                      s += min(50, (cv - 1.5) * 20 + 10)
    if max(diffs) / (mean_d + 1e-8) > 5:
        s += min(50, ((max(diffs) / (mean_d + 1e-8)) - 5) * 5 + 10)
    return round(float(min(100, s)), 1)
...
def run_all_forensic_rules(face_paths, **_):
    ...
    scores = {
        ...
        "ela":         sum(ela_score(p)      for p in s) / n,
        "gradient":    sum(gradient_score(p) for p in s) / n,
    ...
    W = {
        ...
        "noise":       0.15,
        "boundary":    0.15,
        "ela":         0.18,   # strong signal for AI-generated images
        "gradient":    0.12,
        "color":       0.05,
        "temporal":    0.05,
    }
    ...
    return {
        ...
        "anomalies": [f"{k.capitalize()} anomaly: {v:.0f}%"
                      for k, v in scores.items() if v > 30],
    }
